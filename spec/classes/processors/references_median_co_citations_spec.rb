...
describe Processors::ReferencesMedianCoCitations do
  ...
  it "should include median co citations" do
    ...
    expect(result[:references][2][:median_co_citations]).to eq 2
  end

  it "should exclude itself" do
    body "#{cite(1)}"
    expect(result[:references][1][:median_co_citations]).to eq 0
  end

  it "should not be present if it is not cited" do
    body "#{cite(1)}"
    expect(result[:references][2][:median_co_citations]).to be_nil
  end
